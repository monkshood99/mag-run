...
class Addon extends AC\Addon {
	...
	public function ajax_get_options() {
		...
		$column = $list_screen->get_column_by_name( $input->column );
		...
		if ( ! $editing instanceof PaginatedOptions ) {
			wp_die();
		}

		$options = $editing->get_paginated_options(
			$input->searchterm,
			$input->page,
			$input->item_id
		);

		$response = new Response( $options, ! $options->is_last_page() );
