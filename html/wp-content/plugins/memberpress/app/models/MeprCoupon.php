...
class MeprCoupon extends MeprCptModel {
  ...
  public function apply_discount($price, $is_recurring = true, $force = false) {
    $value = $price;

    // First Payment type only discounts the trial
    // Check if one-time payment ... if so, apply the discount
    if($force || !$is_recurring || $this->discount_mode != 'first-payment') {
  ...
  public function maybe_apply_trial_override(&$obj) {
    if($this->discount_mode=='trial-override') {
      ...
      $obj->trial_amount = $this->apply_discount($obj->price, true, true);
    }
  }
